...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    dice_rolls_first = strategy(0, 0)
    for score in range(goal):
        for oppo_score in range(goal):
            curr_rolls = strategy(score, oppo_score)
            if dice_rolls_first != curr_rolls:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_num, curr_num = 1, 1
    max_aver = make_averaged(roll_dice, total_samples)(1, dice)
    for i in range(9):
        curr_num += 1
        curr_aver = make_averaged(roll_dice, total_samples)(curr_num, dice)
        if max_aver < curr_aver:
            max_aver = curr_aver
            max_num = curr_num
    return max_num
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    update_score = square_update(0, score, opponent_score)
    diff = update_score - score
    if diff >= threshold:
        return 0
    else:
        return num_rolls
